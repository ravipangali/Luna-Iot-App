...
class _VehicleHistoryShowState extends State<VehicleHistoryShowScreen>
  ...
  Future<void> _selectStartDate() async {
    ...
    final DateTime? picked = await showDatePicker(
      ...
      lastDate: today, // Can select today but not tomorrow
    );
    if (picked != null) {
      setState(() {
        startDate = picked;
  ...
  Future<void> _selectEndDate() async {
    ...
    if (startDate!.isAtSameMomentAs(
      ...
      // If start date is yesterday, end date can be yesterday and today
      lastDate = today;
    } else {
      // If start date is before yesterday, end date can be up to 3 days after start date
  ...
  void _calculateTrips() {
    try {
      ...
      final statusEvents = historyData
          ...
          .toList();
      print('Status events found: ${statusEvents.length}');
